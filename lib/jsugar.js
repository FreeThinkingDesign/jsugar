...
var _post = function (domain, api, callback) {
  ...
  if (domain.replace(protocolRegex, '').match(/\//))
  /* istanbul ignore next */
    subpath = domain.replace(protocolRegex, '').replace(/^(.+?)\//, '').replace(/\/$/, '');
  else
    subpath = '';
  ...
  options = {
    ...
    host: domain.replace(protocolRegex, '').replace(/\/.*/, ''),
    port: protocolPort,
    path: path + '?' + api,
    method: 'POST'
  };

  request = protocolHandler.request(options, function (response) {
